...
int main()
{
	...
	uintptr_t framebuffer_address = 0;
	uint64_t result = allocate_memory(
			horizontal_resolution * vertical_resolution * 4, &framebuffer_address);
	if (result != 0)
	{
	...
	while (1)
	{
		...
		result = straylight_graphics_update_framebuffer(framebuffer_address);
		if (result != 0)
		{
