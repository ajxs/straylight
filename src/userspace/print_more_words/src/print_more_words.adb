...
procedure Print_More_Words is
   Result         : Syscall_Result_T;
   File_Handle_Id : File_Handle_Id_T;

   Bytes_To_Read  : constant Unsigned_64 := 10;
   Bytes_Read     : Unsigned_64 := 0;
   Buffer_Address : Address := Null_Address;

   Number_Of_Words : constant Unsigned_64 := 8;
begin
   Straylight.Print_To_Serial
     ("Printing " & Number_Of_Words'Image & " more words." & ASCII.LF);
   ...
   Straylight.Open_File
     ("/Devices/Disk/nine_letter_words.txt",
      File_Open_Mode_Read,
      File_Handle_Id,
      Result);
   if Result /= Syscall_Result_Success then
   ...
   loop
      Straylight.Seek_File (File_Handle_Id, 103148, Result);
      if Result /= Syscall_Result_Success then
...
exception
   when others =>
      Straylight.Log_Error ("Exception occurred in Print_More_Words");
end Print_More_Words;
