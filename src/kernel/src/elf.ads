...
package ELF is
   ...
   ----------------------------------------------------------------------------
   subtype Elf_File_Magic_Number is String (1 .. 4);

   ----------------------------------------------------------------------------
   --  ELF File class
   --  Determines the kind of executable this file represents.
   ----------------------------------------------------------------------------
   type EI_CLASS is (ELFCLASSNONE, ELFCLASS32, ELFCLASS64);
   for EI_CLASS'Size use 8;
   for EI_CLASS use (ELFCLASSNONE => 0, ELFCLASS32 => 1, ELFCLASS64 => 2);

   ----------------------------------------------------------------------------
   --  ELF Data Encoding type.
   --  Determines the endianness of the executable.
   ----------------------------------------------------------------------------
   type EI_DATA is (ELFDATANONE, ELFDATA2LSB, ELFDATA2MSB);
   for EI_DATA'Size use 8;
   for EI_DATA use (ELFDATANONE => 0, ELFDATA2LSB => 1, ELFDATA2MSB => 2);

   ----------------------------------------------------------------------------
   --  ELF File Version.
   --  Denotes the version of the ELF file encoding.
   ----------------------------------------------------------------------------
   type EI_VERSION is (EV_NONE, EV_CURRENT, EV_NUM);
   for EI_VERSION'Size use 8;
   for EI_VERSION use (EV_NONE => 0, EV_CURRENT => 1, EV_NUM => 2);
   ...
   ----------------------------------------------------------------------------
   type EI_ABIVERSION is (Default);
   for EI_ABIVERSION'Size use 8;
   for EI_ABIVERSION use (Default => 0);
   ...
   ----------------------------------------------------------------------------
   type Elf_File_Version is (EV_NONE, EV_CURRENT);
   for Elf_File_Version'Size use 32;
   for Elf_File_Version use (EV_NONE => 0, EV_CURRENT => 1);
   ...
   ----------------------------------------------------------------------------
   type Elf32_File_Header_T is record
   ...
   for Elf32_File_Header_T'Size use 416;
   for Elf32_File_Header_T use
     record
   ...
   ----------------------------------------------------------------------------
   type Elf_Section_Header_Type_T is
     (SHT_NULL,
      SHT_PROGBITS,
      SHT_SYMTAB,
      SHT_STRTAB,
      ...
      SHT_LOUSER,
      SHT_HIUSER);
   for Elf_Section_Header_Type_T use
     (SHT_NULL     => 0,
      SHT_PROGBITS => 1,
      SHT_SYMTAB   => 2,
      SHT_STRTAB   => 3,
   ...
   ----------------------------------------------------------------------------
   type Elf32_Section_Flags_T is record
      SHF_WRITE          : Boolean;
      SHF_ALLOC          : Boolean;
   ...
   for Elf32_Section_Flags_T'Size use 32;
   for Elf32_Section_Flags_T use
     record
   ...
   ----------------------------------------------------------------------------
   type Elf32_Section_Header_T is record
      sh_name      : Elf_String_Table_Offset;
      sh_type      : Elf_Section_Header_Type_T;
   ...
   for Elf32_Section_Header_T'Size use 320;
   for Elf32_Section_Header_T use
     record
   ...
   ----------------------------------------------------------------------------
   type Elf64_File_Header_T is record
      e_ident     : Elf_Identifier;
      e_type      : Elf_File_Type;
      e_machine   : Elf_File_Machine_Type;
      e_version   : Elf_File_Version;
   ...
   for Elf64_File_Header_T'Size use 512;
   for Elf64_File_Header_T use
     record
       e_ident at 0 range 0 .. 127;
       e_type at 16 range 0 .. 15;
       e_machine at 18 range 0 .. 15;
   ...
   PT_NULL    : constant := 16#00000000#;
   PT_LOAD    : constant := 16#00000001#;
   PT_DYNAMIC : constant := 16#00000002#;
   PT_INTERP  : constant := 16#00000003#;
   PT_NOTE    : constant := 16#00000004#;
   PT_SHLIB   : constant := 16#00000005#;
   PT_PHDR    : constant := 16#00000006#;
   PT_TLS     : constant := 16#00000007#;
   PT_LOOS    : constant := 16#60000000#;
   PT_HIOS    : constant := 16#6FFFFFFF#;
   ...
   type Elf64_Program_Header_T is record
      p_type   : Unsigned_32;
      p_flags  : Unsigned_32;
      p_offset : Elf64_File_Offset;
      p_vaddr  : Elf64_Address;
      p_paddr  : Elf64_Address;
      p_filesz : Unsigned_64;
      p_memsz  : Unsigned_64;
      p_flags1 : Unsigned_64;
      p_align  : Unsigned_64;
   end record
   ...
   function Validate_Elf_Header_Magic_Number
     (Magic_Number : Elf_File_Magic_Number) return Boolean
   is (Magic_Number = Character'Val (16#7F#) & "ELF")
   with Inline;
